      ...
      <div className="rounded-[26px] border border-[#ffcab8] bg-[linear-gradient(180deg,#fffaf7_0%,#fff2eb_100%)] p-5 shadow-[0_24px_60px_rgba(135,65,24,0.14)]">
        ...
        <div className="mt-4 grid gap-3 rounded-[22px] bg-white p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <strong className="text-[#2f221c]">{shipping === 0 ? "Free" : formatPrice(shipping)}</strong>
          </div>
          <div className="h-px bg-[#f2ddd5]" />
          <div className="flex items-end justify-between">
            <div>
              <p className="m-0 text-xs uppercase tracking-[0.14em] text-[#ff5a36]">Total</p>
              <strong className="text-3xl text-[#d9481f]">{formatPrice(total + shipping)}</strong>
            </div>
            <span className="rounded-full bg-[#fff1ea] px-3 py-1 text-xs font-semibold text-[#d9481f]">Tax included</span>
          </div>
        </div>

        <Link
          href={items.length === 0 ? "/products" : "/checkout"}
          className={`mt-4 inline-flex w-full items-center justify-center rounded-full bg-[linear-gradient(135deg,#ff5a36,#ff7a00)] px-5 py-3.5 text-center text-base font-semibold text-white shadow-[0_16px_32px_rgba(255,106,0,0.24)] ${items.length === 0 ? "pointer-events-none opacity-60" : ""}`}
