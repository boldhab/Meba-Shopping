...
The startup task runs automatically on project open, and the default script is branch-filtered so it only runs on `feature/*` branches.
...
- By default, it only runs on `feature/*` branches
